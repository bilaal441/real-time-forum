...
function controllFilter(value) {
  ...
  switch (value) {
    case "all":
      ...
      break;
    case "created":
      PostView.render(state.posts.filter((el) => el.id === state.id));
      break;
case "liked-posts":
PostView.render(state.posts.filter((el) => el.userlikes.include(state.username)));
break
default:{
  PostView.render(state.posts.filter((el) => el.categories.includes(value)));
}
  }
}
  ...
  try {
    ...
    switch (data.query) {
      ...
      case "post-dislike":
        ...
        break;
      case "comment-like":
        const commentLiked = await likeAndDislikeComment({
          ...data,
          query: data.query.split("-")[1],
        });
        PostView.renderCommentReaction(commentLiked);
        break;
      case "comment-dislike":
        const commentDisliked = await likeAndDislikeComment({
          ...data,
          query: data.query.split("-")[1],
        });
