...
export const likeDislikePostModdel = async (data) => {
  try {
    ...
    state.posts = state.posts.slice().map((post) => {
      if (post.id === res.id) {
        const {
          title,
          body,
    ...
    return state.posts.find((el) => el.id === res.id);
  } catch (err) {
    throw err;
  }
};

const findPost = (id) state.posts.find((el) => el.id === res.id);

const addCommentModdel = async(data)=>{
try{
  const res = await getJson("react-Post-like-dislike", {
    method: "POST",
    headers: {"Content-Type": "application/json"},
